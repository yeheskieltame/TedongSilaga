...
var RESOLVE_PARAMS = parseAbiParameters("uint8 result");
function getEvmClient(config) {
...
function readString(runtime2, evmClient, contractAddress, functionName) {
...
function onLogTrigger(runtime2, log) {
  runtime2.log("=== Tedong Silaga: Market Resolution Workflow ===");
  try {
    ...
    const dataSourceUrl = decoded.args.dataSourceUrl;
    runtime2.log(`[Step 1] MarketLocked event detected`);
    runtime2.log(`[Step 1] Market: ${marketAddress}`);
    runtime2.log(`[Step 1] Data Source: ${dataSourceUrl}`);
    runtime2.log("[Step 2] Reading market details...");
    const evmClient = getEvmClient(runtime2.config);
    const buffaloA = readString(runtime2, evmClient, marketAddress, "buffaloIdA");
    const buffaloB = readString(runtime2, evmClient, marketAddress, "buffaloIdB");
    const eventName = readString(runtime2, evmClient, marketAddress, "eventName");
...
function initWorkflow(config) {
  ...
  return [
    ...
    cre.handler(httpTrigger, (runtime2) => {
      runtime2.log("HTTP trigger: use Log Trigger for market resolution");
      return "OK";
    })
  ];
}
